            ...
            for name, module in model.named_modules():
                ...
                for subname, submodule in module.named_children():
                    if isinstance(submodule, nn.Conv2d):
                        ...
                        pass
            logger.info(
                f"Using Tensor Parallelism for HunyuanDiT, world_size: {distri_config.world_size} and n_device_per_batch: {distri_config.n_device_per_batch}"
            )
        else:
            logger.info("Not using parallelism for DiT")
        super(HunyuanDiTTP, self).__init__(model, distri_config)
